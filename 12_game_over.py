# 게임 종료 처리
# 성공 - 화면내의 모든 버블이 사라짐
# 실패 - 바닥에 어떤 정해진높이보다 버블이 낮게내려오면 실패
...
def setup():
    ...
    for row_idx, row in enumerate(map):
        for col_idx, col in enumerate(row):
            ...
            bubble = Bubble(image, col, position, row_idx, col_idx)
...
def remove_visited_bubbles():
    bubbles_to_remove = [b for b in bubble_group if (
        b.row_idx, b.col_idx) in visited]
...
pygame.display.set_caption("puzzle_Bubble")
